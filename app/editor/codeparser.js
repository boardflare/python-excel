export function parsePython(rawCode) {
    ...
    const formula = `=LAMBDA(${params}, ${runpyEnv}(LET(range,'Boardflare_Functions'!$A$3:$C$30,XLOOKUP("${name}",INDEX(range,,1),INDEX(range,,3),"Not found")), ${params}))`;
    const runpy = `=${runpyEnv}(Functions[@Code], [@Arg1])`;
    const lambda = `${formula}([@Arg1])`;
    const named = `=${name}("example argument")`;
