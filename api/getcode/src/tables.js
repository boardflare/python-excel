...
export async function getAnonCode(env, partitionKey, rowKey) {
	try {
		const response = await fetch(`https://boardflarewest.table.core.windows.net/CodeByTimestamp(PartitionKey='${partitionKey}',RowKey='${rowKey}')${env.CODEBYTIMESTAMP_TABLE_READ}`, {
			method: 'GET',
			headers: {
				'Accept': 'application/json;odata=nometadata',
				'x-ms-date': new Date().toUTCString(),
